...
CREATE TABLE dbo.MA0画面制御(
       ...
       権限設定有無FLG                tinyint NULL DEFAULT 0, 		
       登録日                      smalldatetime NULL, 		
       登録者ID                    numeric NULL, 		
       更新日                      smalldatetime NULL, 		
       更新者ID                    numeric NULL, 		
...
-- テーブル名：MA1画面制御明細
/* ------------------------------------- */
if exists (select * from sysobjects where id = object_id(N'dbo.MA1画面制御明細') and OBJECTPROPERTY(id, N'IsUserTable') = 1)
	drop table dbo.MA1画面制御明細
GO

CREATE TABLE dbo.MA1画面制御明細(
       ID                       numeric NOT NULL IDENTITY(1,1), 	
       担当CD                     int NULL, 		
       画面番号                     nvarchar(4) NULL, 		
       権限                       tinyint NULL DEFAULT 0, 		
       登録日                      smalldatetime NULL, 		
       登録者ID                    numeric NULL, 		
       更新日                      smalldatetime NULL, 		
       更新者ID                    numeric NULL, 		
...
ALTER TABLE dbo.MA1画面制御明細
       ADD CONSTRAINT XPK_MA1画面制御明細 PRIMARY KEY (ID)
GO
