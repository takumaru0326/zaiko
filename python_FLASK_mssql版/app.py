...
def eh1050_get():
    ...
    if "Rq_UserID" in session:
        ...
        strSql = " SELECT ID,アイテムCD,アイテム名,アイテム出庫数,アイテム在庫数,アイテム棚番号,SUBSTRING(アイテム出庫YMD,5,2)+'/'+SUBSTRING(アイテム出庫YMD,7,2),担当名 " \
                "FROM v10SHIP " \
                "WHERE 削除FLG=0 AND LEN(アイテム棚番号)>0 " \
	            " ORDER BY ID DESC" 
...
def eh1060_get():
    ...
    if "Rq_UserID" in session:
        ...
        strSql = " SELECT ID,アイテムCD,アイテム名,アイテム入荷数,アイテム在庫数,アイテム棚番号,FORMAT(登録日, 'MM/dd'),担当名 " \
                "FROM V20INSUU " \
                "WHERE 削除FLG=0 " \
	            " ORDER BY ID DESC" 
