...
def defSqlConnect():
    return sqlite3.connect('c:/zaiko/zaiko.db')

def get_V10SHIP_id(key):
    strSql = "SELECT ID,アイテムCD,アイテム名,アイテム棚番号,アイテム在庫数,アイテム出庫数,担当名 " \
            "FROM V10SHIP " 
    strSql += "WHERE 削除FLG=0 AND ID=" + key 
    rows = executeSelect(strSql)
    return rows[0]

def get_V20INSUU_id(key):
    strSql = "SELECT ID,アイテムCD,アイテム名,アイテム棚番号,アイテム在庫数,アイテム入荷数,担当名 " \
            "FROM V20INSUU " 
    strSql += "WHERE 削除FLG=0 AND ID=" + key 
    rows = executeSelect(strSql)
    return rows[0]
...
def get_M80tanto_id(key):
    strSql = "SELECT ID,担当CD,担当名,ログインCD,担当備考,権限CD " \
            "FROM M80担当者 " 
    strSql += "WHERE 削除FLG=0 AND ID=" + key 
    rows = executeSelect(strSql)
    return rows[0]
...
def eh106UDataApp(vntGid):
    vnt_fnc = request.form['Rq_fnc']
    vnt_id = request.form['EID']
    if vnt_fnc == "add":
        strSql = ""
    elif vnt_fnc == "delete":
        strSql = " UPDATE T20INSUU SET  " \
                    "更新日         =CURRENT_TIMESTAMP," \
                    "更新者ID       ='" +str(session["Rq_UserID"]) + "'," \
                    "削除FLG        =9," \
                    "IPADDR         ='" + request.remote_addr + "'," \
...
def eh10WUDataApp(vntGid):
    ...
    if vnt_fnc == "add":
        ...
        strSql = " UPDATE M80担当者 SET  " \
                    ...
                    "ログインCD   ='"  + request.form['Rq_BosyuCd'] + "'," \
