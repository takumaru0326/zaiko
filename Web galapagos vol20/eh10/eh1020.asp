...
'	connectDB.Execute ("SET IMPLICIT_TRANSACTIONS ON;")	
	...
	'Call fncMessage( "登録完了しました。","eh1020.asp?fnc=add","_top")
	Response.Redirect "eh1020.asp?fnc=add"

End If

If SYORIFLG="reset" Then
	fnc_CookieReset() 
	Response.Redirect "eh1020.asp"
End If
...
<FORM NAME="frmSubmit" ACTION="eh<%= Session("gid") %>.asp" METHOD="post" autocomplete="off">
	...
	<input id="id_ItemCd" type="tel" NAME="Rq_ItemCd" SIZE="30" MAXLENGTH="6" VALUE="<%=Rq_ItemCd %>" style="font-size: 36px;width: 4em;max-width: 100%;" onkeyup="setNextFocus(this)" tabindex="1">
	</div>
	...
	<tr>
	<td>
	<div style="font-size: 30px;">【在庫数】<SPAN ID="id_ItemZaikoSuu"></SPAN>【下限】<SPAN ID="id_ItemMinSuu"></SPAN></div>
	</td>
	</tr>
	
	<tr>
	<td>
	<div style="font-size: 30px;color:green">【入荷数】
	<input id="id_ItemSuu" type="number" NAME="Rq_ItemSuu" SIZE="8" MAXLENGTH="6" VALUE="<%=Rq_ItemSuu%>" style="font-size: 36px;width: 4em;max-width: 100%;ime-mode:inactive" onkeypress="if(event.keyCode==13){frmSubmit.elements['Rq_ItemRCd'].focus()};" tabindex="2">
	</div>
	</td>
	</tr>
	...
	<input id="id_ItemRCd" type="tel" NAME="Rq_ItemRCd" SIZE="30" MAXLENGTH="6" VALUE="<%=Rq_ItemCd %>" style="font-size: 36px;width: 4em;max-width: 100%;" onkeyup="setNextFocus(this)" tabindex="3"></div>
...
function setNextFocus(obj){
	if(obj.value.length >= obj.maxLength){
		if (obj.id == "id_ItemRCd"){
			if (document.forms['frmSubmit'].Rq_ItemCd.value == document.forms['frmSubmit'].Rq_ItemRCd.value){
				add_submit();
			} else {
...
jQuery(function($){
	...
	$('#id_ItemCd').change(function(){
		...
		$.getJSON(url, query, function(json){
			...
			$('#id_ItemMinSuu').text(json.itemMinSuu);
...
%>
		...
		With Response
		 If Not rsSql.EOF Then
			...
			.Write "<TR >"
			.Write "<TH NOWRAP width=""80"">アイテムコード</TH>" 
			.Write "<TH NOWRAP width=""200"">アイテム名</TH>" 
			.Write "<TH NOWRAP width=""50"">入荷数</TH>" 
			...
			Do While startRECORD+EVCNT>intCont And Not rsSql.EOF
				...
				If startFLG = 1 Then
					...
					.Write "<TD style=""text-align:right"">" &  FormatNumber(rsSql("アイテム入荷数"),0,0) & "</TD>"
					.Write "<TD style=""text-align:right"">" &  FormatNumber(rsSql("アイテム在庫数"),0,0) & "</TD>"
					.Write "<TD style=""text-align:right"">" &  rsSql("アイテム棚番号") & "</TD>"
					.Write "<TD>" 
...
SUB fnc_SearchSet()
	...
	sqlwhere = sqlwhere + "AND アイテム入荷YMD='"	+ fncTodayYMD() +		"' "
	sqlwhere = sqlwhere + "AND 担当CD='"+pvntUSER_EigyouCD+"' "
...
Function fnc_DataGet(inKey)
	...
	If Not rsSql.EOF Then
'		Rq_Id		ID	
		Rq_ItemCd		=rsSql("アイテムコード")
		Rq_ItemSuu		=rsSql("アイテム入荷数")
		eid				=rsSql("ID")
	End If
...
Function fnc_DataAdd()	
	...
	strSql = "INSERT INTO T20INSUU("
	strSql = strSql + "アイテムCD,"
	strSql = strSql + "アイテム入荷数,"
	strSql = strSql + "アイテム入荷YMD,"
	strSql = strSql + "担当CD,"
	strSql = strSql + "登録日,"
	strSql = strSql + "登録者ID,"
