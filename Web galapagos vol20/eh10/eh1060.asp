...
If SYORIFLG="L_update" Then
	...
	%>
	<SCRIPT LANGUAGE="JavaScript" >
	top.location.href = 'eh106U.asp?fnc=up&PAGEOLG=<%=Request("PAGEOLG")%>';
	</SCRIPT>
	<%
	Response.End
End If
...
Rq_GamenNextLv = fncDbDataWGet("MA0画面制御","権限LV","画面番号='106U' AND 削除FLG=0",Const_DbAfterOpen)
...
%>
		...
		With Response
		 If Not rsSql.EOF Then
			...
			.Write "<TH NOWRAP width=""60"">入荷数</TH>" 
			...
			Do While startRECORD+EVCNT>intCont And Not rsSql.EOF
				...
				If startFLG = 1 Then
					...
					End If
					.Write "<TD style=""font-size: 24px;text-align:right;"">" & rsSql("アイテム入荷数") & "</TD>"
					.Write "<TD style=""font-size: 24px;text-align:right;"">" & rsSql("アイテム在庫数") & "</TD>"
